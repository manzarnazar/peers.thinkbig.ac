...
<script src="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/js/toastr.min.js"></script>
    ...
    @php 
       $user_info = App\Models\User::where('id', $job_details->user_id)->first();
       $apply = App\Models\JobApply::where('user_id', auth()->user()->id)->where('job_id', $job_details->id)->first();

    @endphp


    <div class="single-item-countable single-entry" id="postIdentification">
        <div class="entry-inner">
            <div class="entry-header d-flex justify-content-between j_head ">
                <div class="ava-info d-flex align-items-center">
                        ...
                        <h3 class="mb-0 d-flex" >
                          <a class="text-black ms-0" href="javascript:;">{{$user_info->name }}</a>
                          <p class="dash-text">{{get_phrase('added a job')}}</p>
                        </h3>
                        <small class="meta-time text-muted">
                            {{ date('M d ', strtotime($job_details->created_at)) }}
                            at {{ date('g:i A', strtotime($job_details->created_at)) }}
                        </small>                        
                     </div>
                </div>
                <a href="{{url()->previous()}}" class="btn_1 common_btn"><i class="fa-solid fa-left-long me-2"></i>{{get_phrase('Back')}}</a>
            </div>
            <div class="row">
                <div class="col-12 single-item-countable">
                     <div class="photoGallery initialized mt-12 single-entry job-single details_job">
                            <div class="entry-img thumbnail-210-200" style="background-image: url('{{ get_job_image($job_details->thumbnail,'thumbnail') }}')">
                                <span class="date-meta">{{currency($job_details->starting_salary_range)}} - {{currency($job_details->ending_salary_range)}} {{get_phrase('/ Monthly')}}</span>
                
                                @php
                                   $wishlist = App\Models\JobWishlist::where('user_id', auth()->user()->id)->where('job_id', $job_details->id)->first();
                                @endphp
                                 <span class="heart-icon icon2" id="wishlist-icon{{$job_details->id}}" onclick="follow_toggle('{{route('job.follow',['id'=>$job_details->id, 'user_id' => auth()->user()->id])}}','{{$job_details->id}}')">
                                    @if(!$wishlist)
                            ...
                            <div class="intro-job apply_job">
                                <div class="see_applicant d-flex justify-content-between align-item-center">
                                    <h4 class="h4">{{$job_details->title}}</h4>
                                    @if($user_info && $user_info->id == auth()->user()->id)
                                       <a class="btn_1" href="javascript:;" data-bs-toggle="modal" data-bs-target="#exampleModal">{{get_phrase('See Applicant')}}</a>
                                    @endif
                                </div>
                                <ul class="job-loc items-local">
                                    <li>
                                        <span>
                                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                </svg> {{get_phrase('Location')}}
                                        </span>
                                        <p class="mb-0">{{$job_details->location}}</p>
                                    </li>
                                    <li>
                                        <span>
                                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <p class="mb-0">{{$job_details->type}}</p>
                                    </li>
                                    <li>
                                        <span>
                                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M18.3334 10.0001C18.3334 14.6001 14.6 18.3334 10 18.3334C5.40002 18.3334 1.66669 14.6001 1.66669 10.0001C1.66669 5.40008 5.40002 1.66675 10 1.66675C14.6 1.66675 18.3334 5.40008 18.3334 10.0001Z" stroke="#8C8A95" stroke-linecap="round" stroke-linejoin="round"/>
                                                <path d="M13.0917 12.65L10.5083 11.1083C10.0583 10.8416 9.69165 10.2 9.69165 9.67497V6.2583" stroke="#8C8A95" stroke-linecap="round" stroke-linejoin="round"/>
                                                </svg>
                                                 {{get_phrase('Status')}}
                                        </span>
                                        <p class="mb-0">
                                          
                                                @if($job_details->status == '1' && $job_details->start_date <= now() && $job_details->end_date >= now())
                                                    {{get_phrase('Open')}}
                                                @elseif($job_details->status == '1' && now() < $job_details->start_date )
                                                    {{get_phrase('Holding')}}
                                                @elseif($job_details->status == '1' && $job_details->end_date  <  now() )
                                                    {{get_phrase('Expire')}}
                                                @else
                                                    {{get_phrase('Pending')}}
                                                @endif
                                ...
                                <div class="job_description">
                                    <div class="all-text">{!! $job_details->description !!}</div>
                                    <a href="#" class="view-text" onclick="toggleText()">{{get_phrase('View More')}}</a>
                                </div>
                            </div>
                               <div class="job-btn">
                                    @if($job_details->status == '1' && $job_details->start_date <= now() && $job_details->end_date >= now() )
                                        @if($apply && $apply->user_id == auth()->user()->id)
                                            <a href="javascript:;" class="btn apply info w-100">{{get_phrase('Applied')}}</a>
                                            @else
                                                @if($user_info && $user_info->id == auth()->user()->id)
                                                <a href="javascript:;" class="btn apply btn-primary w-100">{{get_phrase('You Can"t Apply Your Own Job!')}}</a>
                                                @else
                                                <a href="{{route('job.apply.form',$job_details->id)}}" class="w-100 btn apply_ed apply btn-primary">   {{get_phrase('Apply')}}</a>
                                            @endif
                                        @endif
                                    @elseif($job_details->status == '1' && now() < $job_details->start_date  )
                                        <a href="#" class="btn w-100 apply btn-info">{{ get_phrase('Holding') }}</a>
                                    @elseif($job_details->status == '1' && $job_details->end_date  <  now())
                                        <a href="#" class="btn apply w-100 btn-warning">{{ get_phrase('Expire') }}</a>
                                    @else
                                        <a href="javascript:;" class="w-100  btn apply btn-pending2">{{get_phrase('Pending')}}</a>
                                 @endif
                          </div>
                      </div>
                </div>
    ...
    <div class=" blog-cards bg-white mt-3 radius-8 p-20 mb-12" >
        <div class="row" id="jobdatashow">
            <h4 class="h4 mt-0 relate">{{get_phrase('Related Jobs')}}</h4>
            @foreach($related_jobs as $job) 
            <div class="col-12 col-sm-6 col-md-4 col-lg-4 h-100 my-1 single-item-countable">
  ...
  <!-- Modal -->
  <div class="modal fade job_modal" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <div class="">
                ...
                <table class="table table-borderless">
                    ...
                    <tbody>
                        ...
                        @php
                            $user_info = App\Models\User::where('id', $applicant->user_id)->first();
                        @endphp
                            <tr class="single-row table-light light-row job_ow " id="id-{{$applicant->id}}">
                                <th scope="row">
                                    <div class="single-cell ps-md-3">
                                        <div class="cell-item">
                                            <p>{{++$key}}</p>
                                        </div>
                                    </div>
                                ...
                                <td>
                                    <div class="single-cell">
                                        <div class="cell-item">
                                            <a href="{{route('job.pdf.download', ['id' => $applicant->id])}}" target="_blank" class="btn download_btns  btn-primary"><i class="fa-solid fa-upload"></i></a>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="single-cell">
                                        <div class="cell-item">
                                            <a href="javascript:void(0)" onclick="confirmAction('<?php echo route('job.apply.list-delete', ['id' => $applicant->id]); ?>', true)" class="btn trash"><i class="fa-solid fa-trash"></i></a>
                                        </div>
                                    </div>
                                </td>
                            </tr>
...
<script>
    function toggleText() {
        var allText = document.querySelector('.job_description .all-text');
        var viewText = document.querySelector('.job_description .view-text');

        if (allText.style.maxHeight) {
