                    ...
                    <div class="col-12 col-sm-6 col-md-4 col-lg-4 h-100 my-1 single-item-countable" id="job-{{$job->id}}">
                        <article class="single-entry job-single h-100 p-0">
                            <div>
                                ...
                                @endphp
                                    ...
                                    <div class="pay-btn">
                                        ...
                                        <a href="javascript:;" class="btn">{{get_phrase('Paid')}}</a>
                                        @else
                                            <a href="{{ route('job.pay.form', ['jobId' => $job->id]) }}" class="btn btn-primary">{{get_phrase('Pay')}}</a>
                                        @endif
                                    </div>
                            ...
                            <ul class="items-local">
                                ...
                                <ul class="d-flex job-btn">
                                    @if($job->status == '1' && $job->start_date <= now() && $job->end_date >= now())
                                        <li><a href="#" class="btn common_btn_2">{{ get_phrase('Active') }}</a></li>
                                    @elseif($job->status == '1' && now() < $job->start_date  )
                                        <li><a href="#" class="btn common_btn">{{ get_phrase('Holding') }}</a></li>
                                    @elseif($job->status == '1' && $job->end_date  <  now())
                                        <li><a href="#" class="btn common_btn">{{ get_phrase('Expire') }}</a></li>
                                    @else
                                        <li><a href="#" class="btn common_btn">{{ get_phrase('Pending') }}</a></li>
                                    @endif
                            
                                    <li><a href="{{route('job.edit',$job->id)}}" class="btn edit">
                                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M14.6751 3.21479C14.3887 2.9284 13.9325 2.9284 13.6462 3.21479L8.11788 8.74323C8.03052 8.82576 7.97228 8.93253 7.93345 9.04902L7.03068 12.0632C6.95301 12.3205 7.02582 12.5971 7.21512 12.7864C7.351 12.9272 7.5403 13 7.7296 13C7.79753 13 7.87035 12.9903 7.9383 12.9709L10.9524 12.0681C11.0689 12.0292 11.1757 11.971 11.2582 11.8836L16.7864 6.35517C16.9224 6.21928 17 6.03484 17 5.84068C17 5.64654 16.9224 5.4621 16.7864 5.32618L14.6751 3.21479Z" fill="black"/>
                                            <path d="M16.3226 9.32258C15.9484 9.32258 15.6452 9.62601 15.6452 10V14.5161C15.6452 15.1389 15.1386 15.6452 14.5161 15.6452H5.48387C4.86136 15.6452 4.35484 15.1389 4.35484 14.5161V5.48387C4.35484 4.86114 4.86136 4.35484 5.48387 4.35484H10C10.3742 4.35484 10.6774 4.05141 10.6774 3.67742C10.6774 3.30343 10.3742 3 10 3H5.48387C4.11426 3 3 4.11448 3 5.48387V14.5161C3 15.8855 4.11426 17 5.48387 17H14.5161C15.8857 17 17 15.8855 17 14.5161V10C17 9.62601 16.6968 9.32258 16.3226 9.32258Z" fill="black"/>
                                            </svg></a></li>
                                    <li><a href="javascript:void(0)" onclick="confirmAction('<?php echo route('job.delete', ['job_id' => $job->id]); ?>', true)" class="btn edit"><svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g clip-path="url(#clip0_565_499)">
                                        <path d="M4.99984 15.8333C4.99984 16.7542 5.74567 17.5 6.6665 17.5H13.3332C14.254 17.5 14.9998 16.7542 14.9998 15.8333V5.83333H4.99984V15.8333ZM15.8332 3.33333H12.9165L12.0832 2.5H7.9165L7.08317 3.33333H4.1665V5H15.8332V3.33333Z" fill="black"/>
                                        </g>
                                        <defs>
