            ...
            <div class="row">
                <div class="col-md-6 col-md-6 col-sm-6">
                    <div class="eForm-layouts">
                      <form method="POST" action="{{ route('admin.update.job.category',$jobcategories->id) }}" enctype="multipart/form-data">
                        @csrf
                          <div class="mb-3">
                            <label for="jobcategory" class="form-label eForm-label">{{ get_phrase('Job Category') }}</label>
                            <input type="text" class="form-control eForm-control" id="jobcategory" value="{{ $jobcategories->name }}" name="jobcategory" placeholder="Job Category">
                          </div>
                          @if ($errors->any())
